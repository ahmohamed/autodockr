...
struct parallel_for : private vinaboost::thread_group {
...
private:
	void loop(sz offset) {
		while(vinaboost::optional<sz> sz_option = get_size(offset)) {
			sz s = sz_option.get();
    ...
    vinaboost::condition cond;
    vinaboost::condition busy;
    bool destructing; // dtor called
    sz size; // size of the vector given to run()
	std::vector<bool> thread_finished;
    sz count_finished; 
	sz num_threads;
	vinaboost::mutex self; // any modification or reading of mutables should lock this first
	vinaboost::optional<sz> get_size(sz offset) {
		vinaboost::mutex::scoped_lock self_lk(self);
        while(!destructing && thread_finished[offset])
            cond.wait(self_lk);
		if(destructing) return vinaboost::optional<sz>(); // wrap it up!
        return size;
    }
};

template<typename F>
struct parallel_for<F, true> : private vinaboost::thread_group {
...
private:
	void loop() {
		while(vinaboost::optional<sz> i = get_next()) {
			(*m_f)(i.get());
			{
				vinaboost::mutex::scoped_lock self_lk(self);
				++finished;
	...
	vinaboost::mutex self; // any modification or reading of mutables should lock this first
	vinaboost::optional<sz> get_next() {
		vinaboost::mutex::scoped_lock self_lk(self);
        while(!destructing && started >= size)
            cond.wait(self_lk);
		if(destructing) return vinaboost::optional<sz>(); // NOTHING
		sz tmp = started;
        ++started;
        return tmp;
    }
