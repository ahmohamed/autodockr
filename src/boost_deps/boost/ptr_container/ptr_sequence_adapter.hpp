...
{    
    ...
    struct sequence_config
    {
        ...
        BOOST_STATIC_CONSTANT(bool, allow_null = vinaboost::is_nullable<T>::value );
    };
    ...
    public:
        ...
        { 
            ...
            BOOST_ASSERT( !::vinaboost::is_null( this->begin() ) );
            return *this->begin(); 
        }
        ...
        reference back()
        {
            ...
            BOOST_ASSERT( !::vinaboost::is_null( --this->end() ) );
            return *--this->end(); 
        }
    ...
    public: // assign, insert, transfer
        ...
        {
            assign( vinaboost::begin(r), vinaboost::end(r ) );
        }
    ...
    public:
        ...
        vinaboost::disable_if< ptr_container_detail::is_pointer_or_integral<Range> >::type
        insert( iterator before, const Range& r )
        {
            insert( before, vinaboost::begin(r), vinaboost::end(r) );
        }
        ...
        BOOST_DEDUCED_TYPENAME vinaboost::disable_if< vinaboost::is_same< Range,
                      BOOST_DEDUCED_TYPENAME PtrSeqAdapter::iterator > >::type
        transfer( iterator before, const Range& r, PtrSeqAdapter& from ) // strong
        {
            transfer( before, vinaboost::begin(r), vinaboost::end(r), from );
        }
    ...
    public: // resize
        ...
        void resize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( vinaboost::next( this->begin(), size ), this->end() );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
                    this->push_back( new BOOST_DEDUCED_TYPENAME 
                                     vinaboost::remove_pointer<value_type>::type() ); 
            }
        ...
        void rresize( size_type size ) // basic
        {
            ...
            if( old_size > size )
            {
                this->erase( this->begin(), 
                             vinaboost::next( this->begin(), old_size - size ) );  
            }
            else if( size > old_size )
            {
                for( ; old_size != size; ++old_size )
    ...
    public:
        ...
        void unique( iterator first, iterator last, Compare comp )
        {
            ...
            for( ; next != last; ++next )
            {
                BOOST_ASSERT( !::vinaboost::is_null(prev) );
                BOOST_ASSERT( !::vinaboost::is_null(next) );
                if( comp( *prev, *next ) )
                {
