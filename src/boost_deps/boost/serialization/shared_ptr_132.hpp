...
inline void serialize(
    Archive & /* ar */,
    vinaboost_132::detail::sp_counted_base_impl<P, D> & /* t */,
    const unsigned int /*file_version*/
){
    // register the relationship between each derived class
    // its polymorphic base
    vinaboost::serialization::void_cast_register<
        vinaboost_132::detail::sp_counted_base_impl<P, D>,
        vinaboost_132::detail::sp_counted_base 
    >(
        static_cast<vinaboost_132::detail::sp_counted_base_impl<P, D> *>(NULL),
        static_cast<vinaboost_132::detail::sp_counted_base *>(NULL)
    );
}
...
inline void save_construct_data(
    ...
    ar << vinaboost::serialization::make_nvp("ptr", t->ptr);
}
...
inline void load_construct_data(
    ...
    P ptr_;
    ar >> vinaboost::serialization::make_nvp("ptr", ptr_);
    // ::new(t)vinaboost_132::detail::sp_counted_base_impl<P, D>(ptr_,  D()); 
    // placement
    ...
    // new system as well.
    ::new(t)vinaboost_132::detail::sp_counted_base_impl<
        P, 
        vinaboost_132::serialization::detail::null_deleter
    >(
        ptr_,  vinaboost_132::serialization::detail::null_deleter()
    ); // placement new
...
inline void save(
    Archive & ar,
    const vinaboost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar << vinaboost::serialization::make_nvp("pi", t.pi_);
}

template<class Archive>
inline void load(
    Archive & ar,
    vinaboost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar >> vinaboost::serialization::make_nvp("pi", t.pi_);
    if(NULL != t.pi_)
...
BOOST_SERIALIZATION_SPLIT_FREE(vinaboost_132::detail::shared_count)
...
inline void save(
    Archive & ar,
    const vinaboost_132::shared_ptr<T> &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
    ar.register_type(static_cast<
        vinaboost_132::detail::sp_counted_base_impl<T *, vinaboost::checked_deleter<T> > *
    >(NULL));
    ar << vinaboost::serialization::make_nvp("px", t.px);
    ar << vinaboost::serialization::make_nvp("pn", t.pn);
}

template<class Archive, class T>
inline void load(
    Archive & ar,
    vinaboost_132::shared_ptr<T> &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
...
inline void serialize(
    ...
    BOOST_STATIC_ASSERT(
        ...
        != vinaboost::serialization::track_never
    );
    vinaboost::serialization::split_free(ar, t, file_version);
}
...
#define BOOST_SHARED_POINTER_EXPORT_GUID(T, K)                     \
    typedef vinaboost_132::detail::sp_counted_base_impl<               \
        T *,                                                       \
        vinaboost::checked_deleter< T >                                \
    > __shared_ptr_ ## T;                                          \
