...
#include <boost/type_traits/is_floating_point.hpp> // for vinaboost::is_floating_point;
#include <boost/type_traits/is_integral.hpp> // for vinaboost::is_integral
#include <boost/type_traits/is_convertible.hpp> // for vinaboost::is_convertible
#include <boost/type_traits/is_same.hpp>// for vinaboost::is_same
#include <boost/type_traits/remove_cv.hpp>// for vinaboost::remove_cv
// Boost Template meta programming:
#include <boost/mpl/if.hpp> // for vinaboost::mpl::if_c.
#include <boost/mpl/and.hpp> // for vinaboost::mpl::if_c.
#include <boost/mpl/or.hpp> // for vinaboost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< ::vinaboost::is_convertible<T1P, T2P>, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         BOOST_STATIC_ASSERT((0 == ::vinaboost::is_same<type, long double>::value));
#endif
      };
