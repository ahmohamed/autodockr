...
using vinaboost::system::error_code;
using vinaboost::system::system_category;
...
#     define BOOST_STATVFS_F_FRSIZE static_cast<vinaboost::uintmax_t>( vfs.f_bsize )
#   endif
#   include <dirent.h>
#   include <unistd.h>
#   include <fcntl.h>
...
# endif // ifndef BOOST_FILESYSTEM_NARROW_ONLY
  ...
  template< class String >
  vinaboost::filesystem::detail::query_pair
  ...
  template< class String >
  vinaboost::filesystem::detail::uintmax_pair
  file_size_template( const String & ph )
  {
    ...
    return std::make_pair( ok,
      (static_cast<vinaboost::uintmax_t>(fad.nFileSizeHigh)
  ...
  template< class String >
  vinaboost::filesystem::detail::space_pair
  space_template( String & ph )
  {
    ULARGE_INTEGER avail, total, free;
    vinaboost::filesystem::detail::space_pair result;
    if ( get_free_disk_space( ph, &avail, &total, &free ) )
    {
      result.first = ok;
      result.second.capacity
        = (static_cast<vinaboost::uintmax_t>(total.HighPart) << 32)
          + total.LowPart;
      result.second.free
        = (static_cast<vinaboost::uintmax_t>(free.HighPart) << 32)
          + free.LowPart;
      result.second.available
        = (static_cast<vinaboost::uintmax_t>(avail.HighPart) << 32)
          + avail.LowPart;
    }
    else
    {
  ...
  get_current_path_template( String & ph )
  {
    ...
    vinaboost::scoped_array<value_type> buf( new value_type[sz] );
  ...
  get_full_path_name_template( const String & ph, String & target )
  {
    ...
    {
      typedef typename String::value_type value_type;
      vinaboost::scoped_array<value_type> big_buf( new value_type[len] );
...
# endif
  ...
  template<class String>
  vinaboost::filesystem::detail::time_pair
...
#     ifndef BOOST_FILESYSTEM_NARROW_ONLY
        ...
        const std::wstring & ph ) // return is empty if fails
      {
        ...
        {
          vinaboost::scoped_array<wchar_t> buf( new wchar_t[buf_sz] );
        ...
        vinaboost::scoped_array<char> narrow_buf( new char[narrow_sz] );
...
#   else // BOOST_POSIX_API
      ...
      file_size_api( const std::string & ph )
      {
        ...
        return std::make_pair( ok,
          static_cast<vinaboost::uintmax_t>(path_stat.st_size) );
      }
      ...
      space_api( const std::string & ph )
      {
        ...
        else
        {
          result.first = ok;
          result.second.capacity 
            = static_cast<vinaboost::uintmax_t>(vfs.f_blocks) * BOOST_STATVFS_F_FRSIZE;
          result.second.free 
            = static_cast<vinaboost::uintmax_t>(vfs.f_bfree) * BOOST_STATVFS_F_FRSIZE;
          result.second.available
            = static_cast<vinaboost::uintmax_t>(vfs.f_bavail) * BOOST_STATVFS_F_FRSIZE;
        }
        return result;
      }
...
#         endif
        ...
        const std::string & to_file_ph, bool fail_if_exists )
      {
        ...
        vinaboost::scoped_array<char> buf( new char [buf_sz] );
