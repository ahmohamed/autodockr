...
using namespace vinaboost::spirit;
...
struct append_string {
    ...
    #if 0
        typedef vinaboost::archive::iterators::xml_unescape<Iterator> translator;
        contents.append(
...
bool basic_xml_grammar<CharType>::my_parse(
    ...
    vinaboost::io::ios_flags_saver ifs(is);
    is >> std::noskipws;
    ...
    parse_info<BOOST_DEDUCED_TYPENAME std::basic_string<CharType>::iterator> 
        result = vinaboost::spirit::parse(arg.begin(), arg.end(), rule_);
    return result.hit;
}
